...
DB_PATH = os.environ.get(
    "RAY_LOG_INDEX_DB", str(Path.home() / ".ray_logs_index.sqlite")
)
...
def _infer_component(filename: str) -> str:
    ...
    clean_name = re.sub(
        r"[_-][0-9a-fA-F]{8,}.*$", "", name
    )  # Remove _HASH or -HASH suffixes
...
def _calculate_signal_priority(text: str, filename: str) -> int:
    ...
    if TASK_CONTEXT_PAT.search(text) and (
        "error" in text.lower()
...
def _chunk_file_bytes(
    path: Path,
) -> List[Tuple[int, int, str, Optional[float], Optional[float], int]]:
    """
        ...
        while start < file_size:
            ...
            if traceback_start != -1:
                ...
                for i, line in enumerate(lines):
                    ...
                    if i > 3 and TS_PAT.match(
                        line.strip()
                    ):  # Wait at least 3 lines before considering end
                        # Check if this line looks like start of new log entry vs continuation
                        if not any(
                            marker in line.lower()
                            for marker in ["file ", "line ", "in ", "  "]
                        ):
                    ...
                    # Also end if we see Ray internal logs that aren't part of user exception
                        ...
                        marker in line
                        for marker in ["[I ", "[W ", "[E ", "] (raylet)", "] (gcs"]
                    ):
                        traceback_end = traceback_start + len(
                            "\n".join(accumulated_lines[:-1])
                        )
                ...
                if exception_content_size > CHUNK_BYTES:
                    cut = min(
                        traceback_end, CHUNK_BYTES * 4
                    )  # Allow up to 4x for critical exceptions
                else:
                    cut = traceback_end
            else:
                # Normal chunking logic - but still check for user code patterns
                if USER_CODE_PAT.search(text_preview) or TASK_CONTEXT_PAT.search(
                    text_preview
                ):
...
def _index_logs(conn: sqlite3.Connection, log_dir: str) -> Dict[str, Any]:
    ...
    for path in sorted(root.glob("**/*")):
        ...
        stat = path.stat()
        cur = conn.execute(
            "SELECT id, mtime FROM files WHERE path = ?", (str(path),)
        ).fetchone()
        ...
        if cur:
            ...
            conn.execute(
                "UPDATE files SET mtime = ? WHERE id = ?", (stat.st_mtime, file_id)
            )
        else:
            conn.execute(
...
def _limit_output(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for it in items[:MAX_RESULTS]:
        ...
        total_chars += (
            len(it.get("content", "")) if isinstance(it.get("content", ""), str) else 0
        )
        out.append(it)
            ...
            if force_clean:
                ...
                for file_id, path in file_rows:
                    if not Path(path).exists():
                        ...
                        conn.execute(
                            "DELETE FROM chunks_meta WHERE file_path = ?", (path,)
                        )
    ...
    def _impl():
        ...
        try:
            ...
            if start_ts is not None:
                where.append(
                    "(start_ts IS NOT NULL AND end_ts IS NOT NULL AND end_ts >= ?)"
                )
            ...
            for r in rows:
                priority_label = (
                    "high" if r[8] >= 3 else "medium" if r[8] >= 2 else "low"
                )
                results.append(
                    {
                        "file": r[2],
...
@mcp.tool()
def tail_logs(
    component: Optional[str] = None, N: int = 200, time_range: Optional[str] = None
) -> Dict[str, Any]:
    """
        ...
        try:
            ...
            if not component:
                ...
                components_involved = list(
                    set(comp for _, _, comp in timestamped_lines[:N])
                )
    ...
    def _impl():
        ...
        try:
            ...
            for content, comp_name, signal_priority in rows:
                ...
                component_sigs[comp_name][norm] = (
                    component_sigs[comp_name].get(norm, 0) + 1
                )
            ...
            for comp_name, sig_counts in component_sigs.items():
                items = sorted(sig_counts.items(), key=lambda kv: kv[1], reverse=True)[
                    :20
                ]  # top 20 per component
                clusters = [
                    {
                        "signature": k,
                        "count": v,
                        "example": examples[comp_name].get(k, ""),
                    }
                    for k, v in items
                ]
    ...
    def _impl():
        ...
        try:
            ...
            critical_patterns = [
                (
                    "task_failures",
                    'RuntimeError OR ValueError OR TypeError OR "Task failed" OR "💥" OR "demo crash" OR "raised"',
                ),
                ...
                (
                    "node_failure",
                    "(node* AND failed) OR (worker* AND died) OR (raylet* AND died)",
                ),
                (
                    "network_issues",
                    "(connection* AND failed) OR timeout OR unreachable",
                ),
            ]
            ...
            for pattern_name, pattern in critical_patterns:
                try:
                    ...
                    """,
                        (one_hour_before_latest, pattern),
                    ).fetchone()[0]
                    if count > 0:
            ...
            return {
                "log_timing": {
                    "latest_log_time": datetime.fromtimestamp(
                        latest_log_ts
                    ).isoformat(),
